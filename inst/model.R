...
params <- list(
  ...
  , "eta" = 0.005 # Step-size
  , "max_depth" = 8 # How many splits in my trees
)
...
# Build the model
SpeciesPredict <- xgboost(param=params,data = x 
                        , label = y, nrounds = nround
                        , missing = NA)

# Save the model
xgb.save(SpeciesPredict,"irisSpecies.model")
...
xgb.importance(var.names, model = SpeciesPredict)
